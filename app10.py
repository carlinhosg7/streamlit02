...
# 📦 Botão para Calcular Giro de Estoque
st.markdown("---")
st.markdown("### 📦 Cálculo de Giro de Estoque / Sell-Out")

arquivo_giro = "Giro_Estoque_Calcados_Formulas_OK.xlsx"  # caminho ou URL do arquivo com estoque

if st.button("📊 Calcular Giro / Sell-Out"):
    try:
        df_giro = pd.read_excel(arquivo_giro)

        # Simula vendas (ou adapte para dados reais)
        np.random.seed(42)
        df_giro["Vendas_Periodo"] = np.random.randint(20, 200, size=len(df_giro))
        df_giro["Dias_Periodo"] = 30  # assumindo 30 dias

        df_giro["Estoque_Medio"] = df_giro["Estoque Atual"] / 2
        df_giro["Giro_Estoque"] = df_giro["Vendas_Periodo"] / df_giro["Estoque_Medio"]
        df_giro["Dias_Cobertura"] = df_giro["Estoque Atual"] / (df_giro["Vendas_Periodo"] / df_giro["Dias_Periodo"])

        st.success("✅ Cálculo realizado com sucesso!")
        st.dataframe(df_giro[[
            "Nome Produto", "Categoria", "Estoque Atual", "Vendas_Periodo",
            "Estoque_Medio", "Giro_Estoque", "Dias_Cobertura"
        ]])
